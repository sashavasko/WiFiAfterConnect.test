package com.wifiafterconnect.test.javascript;
...
import com.wifiafterconnect.test.JavaScriptTests;
...
public class GuestNet1JSTesters extends JavaScriptTests {
	...
	public static final String  CUSTOM_VARS_DEFAULT =
	...
	public String formatCustomVars (String contrType, String hotelGroup, String hotelBrand, String hotelId, String usr, String pwd, String redir) {
		...
		vars.append("\t\t" + (contrType == null ? "//" : "  ")).append(" ControllerType = \"" + (contrType == null ? "vp" : contrType) + "\";    // Manually Set ControllerType Here\n");
		vars.append("\t\t" + (hotelGroup == null ? "//" : "  ")).append(" HotelGroup = \"" + (hotelGroup == null ? "tr" : hotelGroup) + "\";    // Manually Set Hotel Group Here\n");
		vars.append("\t\t" + (hotelBrand == null ? "//" : "  ")).append(" HotelBrand = \"" + (hotelBrand == null ? "s8" : hotelBrand) + "\";    // Manually Set Hotel Brand Here\n");
		vars.append("\t\t" + (hotelId == null ? "//" : "  ")).append(" HotelId = \"" + (hotelId == null ? "20177" : hotelId) + "\";    // Manually Set Hotel Code Here\n");
		vars.append("\t\t" + (usr == null ? "//" : "  ")).append(" usr = \"" + (usr == null ? "PC103" : usr) + "\";    // Manually Set User Here\n");
		vars.append("\t\t" + (pwd == null ? "//" : "  ")).append(" pwd = \"" + (pwd == null ? "yes" : pwd) + "\";    // Only use \"yes\" or \"no\" to manually indicate that guests need a pass code.\n");
		vars.append("\t\t" + (redir == null ? "//" : "  ")).append(" redir = \"" + (redir == null ? "" : redir) + "\";\n");
	...
	public class GuestNet1Tester implements JavaScriptTester {
		...
		public boolean getGoodTokens(List<Token> tokensClean, int testNo) {
			...
			if (testNo == t++) {
				...
				return addAssignShort (tokensClean, "redir", redir, false);
			}else 
				return false;
		}
		...
		public boolean getBadTokens(List<Token> tokensClean, int testNo) {
			...
			if (testNo == t++) {
				...
				return addAssignShort (tokensClean, "redir", redir, true);
			}else					
				return false;
		}
